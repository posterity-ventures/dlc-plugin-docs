...
- **Fix**: Stop the stabilization loop (`Ctrl-C` if in interactive mode, or wait for it to hit its iteration cap). Read the full CI log yourself — `gh run view <run-id> --log-failed`. Then restart stabilization with an explicit diagnosis:
  ...
  ```
- **Prevention**: The [ci-stabilization rule](https://github.com/posterity-ventures/dlc-plugin/blob/main/rules/ci-stabilization.md) caps retries at 3 and requires reading the full log each time. Trust the rule.
...
- **Prevention**: Read [worktree-safety rule](https://github.com/posterity-ventures/dlc-plugin/blob/main/rules/worktree-safety.md) once. The mental model is "each worktree is its own checkout."
...
- **Fix**: Check `.claude/settings.json` for the `hooks` block. Verify the `matcher` regex matches the event type. Test with a trivial `echo` command. See [hooks reference](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/hooks.md) for the matcher syntax.
...
- **Fix**: Check `.claude/hooks/pre-push.sh` or equivalent. Restrict it to unit tests only per the [ci-stabilization rule](https://github.com/posterity-ventures/dlc-plugin/blob/main/rules/ci-stabilization.md).
...
- **Fix**: Per [maintain-docs reference](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/skills/maintain-docs.md), the distinction is **permanent vs transient**. Permanent artifacts (PRD, design, review reports) stay. Transient artifacts (`state.md`, `telemetry.jsonl`, scratch plans) can be removed after merge. `finalize-sdlc` does this automatically — if it hasn't, run `/finalize-sdlc <slug>` on the old runs.
...
- **Fix**: Read the error line in the startup log. It will name the specific var or binary. Check `.env` against [mcp.md](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/mcp.md#environment-variables) for the list. Restart Claude Code after fixing.
...
- **Fix**: Per [image-generation reference](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/skills/image-generation.md), rewrite the prompt to be positive-only ("include a red door" instead of "no green doors"). If that fails, fall back to inline mermaid — it handles most structural diagrams well.
...
See [hotfix skill reference](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/skills/hotfix.md) for the full 11-step flow.
...
File an issue against [posterity-ventures/dlc-plugin](https://github.com/posterity-ventures/dlc-plugin/issues) with the failing `state.md`, the last 100 lines of `telemetry.jsonl`, and a description of what you invoked.
...
- Incident response runbook: [hotfix skill reference](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/skills/hotfix.md).
- Learn the building blocks so you can diagnose faster: [skills-guide concepts](https://github.com/posterity-ventures/dlc-plugin/blob/main/docs/skills-guide/concepts.md).
